...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
    go build -ldflags="-s -w" -o /kubevirt-storage-latency-exporter ./cmd/exporter/

FROM registry.access.redhat.com/ubi9/ubi-minimal:latest

COPY --from=builder /kubevirt-storage-latency-exporter /usr/local/bin/kubevirt-storage-latency-exporter

LABEL name="kubevirt-storage-latency-exporter" \
...
USER 0
ENTRYPOINT ["/usr/local/bin/kubevirt-storage-latency-exporter"]
