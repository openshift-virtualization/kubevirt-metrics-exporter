...
No `kubevirt-metrics-exporter` targets have been scraped by Prometheus for at
...
1. Check the DaemonSet status:
   ...
   ```bash
   kubectl get daemonset kubevirt-metrics-exporter -n <namespace>
   ```
...
2. Identify failing pods and inspect their logs:
   ...
   ```bash
   kubectl get pods -n <namespace> -l app=kubevirt-metrics-exporter
...
5. Confirm Prometheus can reach the exporter endpoint:
   ...
   ```bash
   kubectl get podmonitor -n <namespace> kubevirt-metrics-exporter
   ```
...
- On OpenShift, also verify the SecurityContextConstraint is still bound:
  ...
  ```bash
  oc get scc kubevirt-metrics-exporter
  oc adm policy who-can use scc kubevirt-metrics-exporter
  ```
